	...
	if (foundQuote):
		...
		if (diff < cache_time_limit):
			...
			logInput = {
				...
				'stock': foundQuote['stock'],
				'userid': foundQuote['userid'],
				'quotetimestamp': foundQuote['timestamp'],
				'hash': foundQuote['hash']
			}
			logs.insert_one(logInput)
...
def handle_sell(userid, stock, amount):
	...
	if (rounded_stock_number < 1):
		logInput = {
			...
			'stock': stock,
			'amount': amount,
			'error': 'User did not provide enough to sell 1 unit of stock'
		}
		logs.insert_one(logInput)
		return {
			'success': 0,
	...
	if stock in foundUser['stocks']:
		...
		if (foundUser['stocks'][stock]['units'] < foundUser['pending_sell']['num_stocks']):
			logInput = {
				...
				'command': 'COMMIT_SELL',
				'userid': userid,
				'error': 'User does not currently own enough of this stock.'
			}
			logs.insert_one(logInput)
			return {
				'success': 0,
		...
		logInput = {
			...
			'command': 'COMMIT_SELL',
			'userid': userid,
			'error': 'User does not currently own this stock.'
		}
		logs.insert_one(logInput)
		return {
			'success': 0,
...
def handle_dumplog():
	...
	for log in loglist:
		if (log['logType'] == 'UserCommandType_0' or log['logType'] == 'UserCommandType_1' or log['logType'] == 'UserCommandType_2' or log['logType'] == 'UserCommandType_3' or log['logType'] == 'UserCommandType_4'):
			transactionNum += 1
		logfile += format_log(log, transactionNum)
...
def format_log(data, transactionNum):
	if (data['logType'] == 'QuoteServerType'):
		return ("\t<quoteServer>\n"
			"\t\t<timestamp>{}</timestamp>\n"
			"\t\t<server>{}</server>\n"
			...
			data['hash'].rstrip()))
	elif (data['logType'] == 'UserCommandType_0'):
		return ("\t<userCommand>\n"
			"\t\t<timestamp>{}</timestamp>\n"
			"\t\t<server>{}</server>\n"
			"\t\t<transactionNum>{}</transactionNum>\n"
			...
			data['command'].upper(),
			data['filename']))
	elif (data['logType'] == 'UserCommandType_1'):
		return ("\t<userCommand>\n"
			"\t\t<timestamp>{}</timestamp>\n"
			"\t\t<server>{}</server>\n"
			"\t\t<transactionNum>{}</transactionNum>\n"
			...
			data['command'].upper(),
			data['userid']))
	elif (data['logType'] == 'UserCommandType_2'):
		return ("\t<userCommand>\n"
			"\t\t<timestamp>{}</timestamp>\n"
			"\t\t<server>{}</server>\n"
			"\t\t<transactionNum>{}</transactionNum>\n"
	...
	elif (data['logType'] == 'UserCommandType_3'):
		return ("\t<userCommand>\n"
			"\t\t<timestamp>{}</timestamp>\n"
			"\t\t<server>{}</server>\n"
			"\t\t<transactionNum>{}</transactionNum>\n"
			...
			data['userid'],
			data['stock'].upper()))
	elif (data['logType'] == 'UserCommandType_4'):
		return ("\t<userCommand>\n"
			"\t\t<timestamp>{}</timestamp>\n"
			"\t\t<server>{}</server>\n"
			"\t\t<transactionNum>{}</transactionNum>\n"
			...
			data['stock'].upper(),
			"{:.2f}".format(data['amount'])))
	elif (data['logType'] == 'AccountTransactionType'):
		return ("\t<accountTransaction>\n"
			...
			data['userid'],
			"{:.2f}".format(data['amount'])))
	elif (data['logType'] == 'SystemEventType_1'):
		return ("\t<systemEvent>\n"
			"\t\t<timestamp>{}</timestamp>\n"
			"\t\t<server>{}</server>\n"
			"\t\t<transactionNum>{}</transactionNum>\n"
			...
			data['command'].upper(),
			data['userid']))
	elif (data['logType'] == 'SystemEventType_2'):
		return ("\t<systemEvent>\n"
			"\t\t<timestamp>{}</timestamp>\n"
			"\t\t<server>{}</server>\n"
			"\t\t<transactionNum>{}</transactionNum>\n"
			...
			data['userid'],
			data['stock'].upper()))
	elif (data['logType'] == 'SystemEventType_3'):
		return ("\t<systemEvent>\n"
			"\t\t<timestamp>{}</timestamp>\n"
			"\t\t<server>{}</server>\n"
			"\t\t<transactionNum>{}</transactionNum>\n"
			...
			data['stock'].upper(),
			"{:.2f}".format(data['amount'])))
	elif (data['logType'] == 'SystemEventType_4'):
		return ("\t<systemEvent>\n"
			"\t\t<timestamp>{}</timestamp>\n"
			"\t\t<server>{}</server>\n"
			"\t\t<transactionNum>{}</transactionNum>\n"
			...
			data['stock'],
			"{:.2f}".format(data['price'])))
	elif (data['logType'] == 'ErrorEventType_1'):
		return ("\t<errorEvent>\n"
			"\t\t<timestamp>{}</timestamp>\n"
			"\t\t<server>{}</server>\n"
			"\t\t<transactionNum>{}</transactionNum>\n"
			...
			data['userid'],
			data['error']))
	elif (data['logType'] == 'ErrorEventType_2'):
		return ("\t<errorEvent>\n"
			"\t\t<timestamp>{}</timestamp>\n"
			"\t\t<server>{}</server>\n"
			"\t\t<transactionNum>{}</transactionNum>\n"
			...
			data['stock'].upper(),
			data['error']))
	elif (data['logType'] == 'ErrorEventType_3'):
		return ("\t<errorEvent>\n"
			"\t\t<timestamp>{}</timestamp>\n"
			"\t\t<server>{}</server>\n"
			"\t\t<transactionNum>{}</transactionNum>\n"
